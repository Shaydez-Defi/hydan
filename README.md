...
- **Borrow**: `prepareBorrow(...)` registers two encrypted handles, then `borrow(...)` verifies a TEE-produced decryption proof on-chain, draws USDC from Aave, and records `debtOf[user]` as another private handle. (The borrow _amount_ is public because Aave emits it anyway, but your stored _debt balance_ is not.)
